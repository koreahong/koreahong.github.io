...
- solution
    1. 최고 말단일때는 숫자로 반환
    2. 주어진 식을 0에서부터 끝까지 for문을 거쳐 모든 경우의 수를 추출함
        - operation을 기준으로 왼쪽, 오른쪽을 나눔
    3. 1,2번 반복  
    ```python  
        class Solution:
...
- solution 1  
    - 지금까지 합이 0 이하인 것은 필요없다
    - 양수인 값만 더해서 최대 값을 구하면 된다  
```python 
    class Solution:
...
```python  
    class Solution:
        ...
        stair = collections.defaultdict(int)
        def climbStairs(self, n: int) -> int:
...
```python 
    class Solution:
        def rob(self, nums: List[int]) -> int:
            ...
            money[0], money[1], money[2]  = nums[0], nums[1], nums[2] + nums[0]
