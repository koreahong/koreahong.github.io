...
- solution
    1. 인덱스로 비교
        - 윈도우 숫자만큼 충분한지 확인
        - 윈도우 밤위를 벗어났는지 확인
    2. 현재값이 가장 크다면 계속 빼내기
    3. 윈도우 벗어나면 제거
        - 해당사항은 첫번째 값이 윈도우 범위에 벗어날때까지 클 경우 나타남  
    4. max값 추가  
    ```python  
        class Solution:
            def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
                ...
                for idx, cur in enumerate(nums):
                    #현재 값이 가장 큰 값이면 전부빼내기
                    while q and nums[q[-1]] <= cur:
                        q.pop()
                        
                    q.append(idx)
                    
                    #오른쪽으로 가면서 윈도우에 벗어나면 제거
                    #해당값은 최대값이었던 idx인데 윈도우 범위를 벗어난것
                    if idx - q[0] >= k:
                        q.popleft()
    ...
    ```python 
        class Solution:
            def minWindow(self, s: str, t: str) -> str:
                #Counter를 사용하는 것보다, dict선언 + for문이 훨씬 빠름
                ...
                for right, char in enumerate(s, 1):
                    ...
                    if missing == 0:
                        ...
                        #현재 필요한 문자에 위치함으로 다음으로 넘어가기 위해서 해당 변수들을 + 1해줌
                        need[s[left]] += 1
                        missing += 1
                        left += 1
                return s[start:end] if end <= len(s) else ''
    ```
