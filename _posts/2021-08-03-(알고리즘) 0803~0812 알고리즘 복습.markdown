---
layout: post
title: '(알고리즘) 0803~0812, 파이썬 알고리즘 인터뷰'
...
## 134. Gas Station  
- 답이 한 개 아니면 없다고 문제에서 가정하였기 때문에, 배열 처음부터 해당지점에서 출발이 가능한지만 확인하면 된다.  
```python 
    class Solution:
...
```python 
    class Solution:
        def diffWaysToCompute(self, expression: str) -> List[int]:
            #분할 -> 숫자 한개까지 / None까지
                #return 값은 숫자이거나, None값이 어야 함
            if not expression:
                return None
            if expression.isdigit():
                return [int(expression)]
            #정복 -> operation에 맞춰서 계산
                #부분구간별로 여러 결과가 나옴, operation 연산 순서에 따라서
            results = []
            for index, operation in enumerate(expression):
                if operation in "+-*":
...
# 23장. 다이나믹 프로그래밍
- 하향식 : 메모이제이션, 재귀풀이와 비슷한 방식
- 
- 상향식 : 타뷸레이션, 배열에 저장하면서 작은 문제에서 큰 문제를 푸는 방식
